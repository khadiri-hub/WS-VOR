...
redipsInit = function () {
    ...
    rd.style.opacityDisabled = 80;		// disabled elements will have opacity effect
    // initialization
    rd.init();
    ...
    rd.event.clicked = function () {
        ...
        var id = 'Soc' + pos[2] + '' + pos[1];
        var idNewSociete = $('#' + id).attr('value');//xav -->hind :ca marche mieux que le get elementbyid qui renvoyait 'undefined'
        var nomNewSociete = $('#' + id).attr('title');//pareil ici 
        var idNewCompte = $('#' + id).attr('texte');

        if (!$('#' + id).data("tablemedia"))
            $('#hdnPupDragPos').val(pos[2] + ';' + pos[1]);
        else
            $('#hdnPupDragPos').val("");
        
        $('#hfNewSocieteId').val(idNewSociete);
        $('#hfNewSocieteId').attr('title', nomNewSociete);
        $('#hfNewSocieteId').attr('texte', idNewCompte);
    ...
    rd.event.beforeDropped = function () {
