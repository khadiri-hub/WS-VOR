...
{
    public partial class CommercialUC : BaseUserControl
    {
            ...
            get
            {
                if (ViewState["ListPersonnes"] == null)
                {
                    ...
                    if (this.PelerinId != 0)
                    {
                        ...
                        if (pelerin != null)
                        {
                            foreach (TPersonnePelerin item in pelerin.Personnes.Select(n => n).Where(n => n.Personne.TypePersonne.ID == (int) EnumTypePersonne.COMMERCIAL))
                            {
        ...
        public void GetCommerciaux()
        {
            IList<Personne> lstCommercial = Global.Container.Resolve<PersonneModel>().GetPersonneByTypePersonne((int) EnumTypePersonne.COMMERCIAL);
            if (lstCommercial != null)
            {
                _radListBoxSource.DataSource = lstCommercial;
